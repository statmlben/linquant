...
@cython.wraparound(False)
@cython.nonecheck(False)
def CD(double[:,::1] X, double[::1] y, double[::1] diag, double[::1] alpha, double[::1] beta, double[::1] sample_weight, double[::1] tau, int max_iter, double eps, int print_step):
	...
	for ite in xrange(max_iter):
		...
		for i in xrange(n):
			grad_tmp = run_blas_dot(beta, X[i], d)
			if diag[i] != 0:
				delta_tmp = (y[i] - grad_tmp) / diag[i]
				delta_tmp = max(- alpha[i] - (1 - tau[i]) * sample_weight[i], min(tau[i] * sample_weight[i] - alpha[i], delta_tmp))
			if diag[i] == 0:
				if grad_tmp < 1.:
					...
					delta_tmp = -alpha[i]
			alpha[i] += delta_tmp
			# beta = beta + delta_tmp*X[i]
			run_daxpy(delta_tmp, X[i], beta, d)
